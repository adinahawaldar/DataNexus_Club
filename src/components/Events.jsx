...
function NexathonFlipClockTimer({ googleFormLink, isDark }) {
  ...
  // Intro animation when timer enters viewport
  useEffect(() => {
    if (isInView) {
      const timeLeft = getTimeLeft();
      ...
      const dControls = animate(0, timeLeft.days, {
        duration: 1.8,
        ease: 'easeOut',
        onUpdate: (v) => setDaysDisplay(Math.round(v)),
      });

      const hControls = animate(0, timeLeft.hours, {
        duration: 1.8,
        ease: 'easeOut',
        onUpdate: (v) => setHoursDisplay(Math.round(v)),
      });

      const mControls = animate(0, timeLeft.minutes, {
        duration: 1.8,
        ease: 'easeOut',
        onUpdate: (v) => setMinsDisplay(Math.round(v)),
      });

      const sControls = animate(0, timeLeft.seconds, {
        duration: 1.8,
        ease: 'easeOut',
              ...
              >
                ...
                <ul
                  className={`text-xs space-y-1 list-disc list-inside ${
                    isDark ? 'text-purple-200/90' : 'text-purple-900'
                  }`}
                >
                  <li>Open to all students & tech enthusiasts</li>
                  <li>
                    Certificate of participation & networking opportunities
                  </li>
                  <li>
                    Mentorship from senior AI & Data Science members
                  </li>
                </ul>
              </div>
