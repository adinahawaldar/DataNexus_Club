...
export default function TeamPreview() {
  ...
  return (
    <section
      id="teams"
      className={`relative w-full py-20 sm:py-28 select-none overflow-hidden transition-colors duration-500 ${
        isDark
    ...
    >
      <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 max-w-[1440px] mx-auto text-center flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-12 items-center justify-center">
            {row1Leaders.map((leader, idx) => (
              <motion.div
                ...
                className={`relative w-full max-w-[240px] sm:max-w-[270px] mx-auto aspect-[4/5] rounded-[32px] overflow-visible shadow-2xl ${
                  leader.rotate
              ...
              >
                {/* Photo Container */}
                <div
                  className={`w-full h-full rounded-[28px] overflow-hidden relative flex items-end justify-center ${
                    isDark
                ...
                <div
                  className={`absolute ${
                    leader.badgePos
                  } px-4 py-2.5 sm:px-5 sm:py-3 rounded-full flex flex-col items-center text-center whitespace-nowrap z-30 pointer-events-none transition-colors ${
                    isDark
                ...
                >
                  <span
                    className={`text-[11px] sm:text-xs font-medium tracking-tight leading-none ${
                      isDark ? 'text-purple-300' : 'text-zinc-500'
                    }`}
                  >
                    {leader.role}
                  </span>
                  ...
                  <span
                    className={`text-xs sm:text-sm font-semibold leading-tight mt-1 ${
            ...
            {row2Leaders.map((leader, idx) => (
              ...
              >
                ...
                <div
                  className={`absolute ${
                    leader.badgePos
                  } px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-full flex flex-col items-center text-center whitespace-nowrap z-30 pointer-events-none transition-colors ${
                    isDark
                      ? 'bg-[#1a1436]/95 backdrop-blur-xl border border-purple-400/20 text-white shadow-[0_12px_28px_rgba(0,0,0,0.5)]'
                      : 'bg-[#f4f4f5]/95 backdrop-blur-xl border border-purple-200/40 text-zinc-800 shadow-[0_12px_28px_rgba(0,0,0,0.14)]'
                  }`}
                >
                  <span
                    className={`text-[10px] sm:text-[11px] font-medium tracking-tight leading-none ${
                      isDark ? 'text-purple-300' : 'text-zinc-500'
                    }`}
                  >
                    {leader.role}
                  ...
                  <span
                    className={`text-xs sm:text-xs font-semibold leading-tight mt-1 ${
                      isDark ? 'text-white' : 'text-zinc-800'
                    }`}
                  >
                    {leader.name}
        ...
        >
          <h2
            className={`font-sans font-bold text-3xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-[7.5rem] tracking-tight leading-none whitespace-nowrap flex items-center justify-center gap-2 sm:gap-4 w-full transition-colors ${
          ...
          >
            ...
            <svg
              className={`inline-block w-[0.6em] h-[0.6em] transition-transform duration-300 group-hover:translate-x-2 group-hover:-translate-y-2 flex-shrink-0 ${
                isDark ? 'text-purple-400' : 'text-[#1a073f]'
              }`}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
